...
import { Calendar, Clock, Building2, FileText, Plus, CalendarCheck, PlayCircle, CheckCircle } from 'lucide-react';
    ...
    <div className="min-h-screen" style={{ background: 'var(--background)', color: 'var(--foreground)' }}>
      ...
      <main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <div className="px-4 py-6 sm:px-0">
          <LoadingWrapper loading={loading} loadingText="Загрузка активностей...">
            {error ? (
              <ErrorMessage message={error} className="card rounded-xl p-8 text-center" />
            ) : (
              <>
                <div className="mb-8">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 flex-1 min-w-0">
                            <div className="flex items-center gap-2 text-sm" style={{ color: 'var(--muted-foreground)' }}>
                              <Calendar size={16} />
                              <span className="whitespace-nowrap">{formatters.formatDate(activity.visitDate)}</span>
                              <Clock size={16} />
                              <span className="whitespace-nowrap">{formatters.formatTime(activity.startTime)}-{formatters.formatTime(activity.endTime)}</span>
                            </div>
